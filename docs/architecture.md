...
- **Keys**:
  ...
  - Each must be at least **60 characters**, and `config.Load` refuses to start otherwise. The keys are compared against a caller-supplied bearer token with no rate limiting anywhere in the service, so a short key is brute-forceable online. The floor is set against the deployed values (read 62, write 64). It is a length floor, not an entropy measure — real assurance comes from minting keys with a CSPRNG at provisioning time.
  - Neither may carry leading or trailing whitespace: the middleware trims the presented token, so a padded key either fails to authenticate or silently collides with a differently-padded one.
