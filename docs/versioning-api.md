...
- **DEVOPS-217 review follow-up** — `POST /v1/slips/find-all-by-commits` now carries `X-Slippy-Results-Truncated` on every `200`, valued `true` when the ancestry fallback hit its 256-resolution cap before covering every requested commit and `false` otherwise. huma emits boolean headers unconditionally, and that is the better contract: a caller can distinguish "complete answer" (`false`) from "a build too old to report truncation" (header absent). The response body is byte-identical either way, so no consumer needs to change. Previously the shortfall went only to the service's own log, so a partial answer and a complete one were indistinguishable behind a `200` and a nil error. Only the ancestry fallback can set it — a result served from the direct Postgres lookup answers every commit in one query and is always complete.
