...
- **The client address is resolved from `CF-Connecting-IP`, then `X-Forwarded-For` indexed from the RIGHT — never the left.** Measured against the real edge: Cloudflare *appends* the true client IP to whatever `X-Forwarded-For` the caller sent, so attacker-supplied entries pile up on the left and survive. Reading the leftmost entry — the conventional choice — would let an attacker rotate the header per request and never accrue a penalty at all. `CF-Connecting-IP` cannot be forged through Cloudflare (the edge rejects the attempt with `403`), and `X-Envoy-External-Address` is deliberately **not** used: it carries the Cloudflare edge address rather than the client, and is not even stable per caller.
